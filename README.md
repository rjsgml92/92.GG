...
- 평가 UI: `evaluation.css`, `evaluation-client.js`
...
4. 승패 보너스 없이 초반 주도권 25%, 자원 운영 20%, 교전 영향력 35%, 생존·시야 20%로 한 경기 점수와 등급을 고정 계산합니다.
5. AI 키가 있으면 익명화한 경기 통계만 Gemini로 보내 수치 근거와 코칭 문장을 작성하고, 없으면 같은 근거로 만든 통계 코칭을 표시합니다. AI는 서버가 계산한 점수와 등급을 바꾸지 않습니다.

리포트의 포지션별 기준선은 패치·티어 평균을 가장한 값이 아니라 다음 게임에서 확인하기 위한 넓은 연습선입니다. 팀 오브젝트 이벤트는 개인 관여로 단정하지 않으며, 화면 움직임·스킬 적중·콜처럼 API로 볼 수 없는 정보는 평가 범위에서 제외합니다.
